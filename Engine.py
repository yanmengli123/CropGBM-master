...
from Parameters import check_params
...
def get_params(user_params):
    ...
    # Only add early_stopping_rounds when there's a valid set (LightGBM 4.x requires valid_set for early stopping)
    if user_params['validgeno']:
        lgb_params_dict['early_stopping_rounds'] = user_params['early_stopping_rounds']
...
def lgb_train(params_dict, savedir, user_params):
    ...
    """
    # load params
    traingeno = check_params(user_params, 'traingeno')
    trainphe = check_params(user_params, 'trainphe')
    ...
    # valid set
    if validgeno:
        validphe = check_params(user_params, 'validphe')
...
def lgb_cv(params_dict, user_params):
    ...
    traingeno_data = traingeno_data.loc[trainphe_data.index.values, :]
    train_set = lgb.Dataset(traingeno_data, label=trainphe_data)
    eval_dict = lgb.cv(params_dict, train_set, num_boost_round, nfold=cv_nfold,
                       init_model=init_model_path,
                       stratified=False, shuffle=True)

    if objective == 'regression':
        # LightGBM 4.x uses 'valid l2-mean' key, older versions use 'l2-mean'
        l2_key = [k for k in eval_dict.keys() if 'l2-mean' in k][0]
        eval_array1 = np.array(eval_dict[l2_key])
...
def lgb_predict(user_params, savedir):
    ...
    Return: None
    """
    testgeno = check_params(user_params, 'testgeno')
...
def lgb_iter_feature(bygain_feature_array, trainfile_data, trainphe_data, params_dict,
    ...
    for i in range(1, exfeature_num + 1):
        ...
        if params_dict['objective'] == 'regression':
            l2_key = [k for k in cv_result.keys() if 'l2-mean' in k][0]
            imse = cv_result[l2_key][-1]
        else:
            ml_key = [k for k in cv_result.keys() if 'multi_logloss-mean' in k][0]
            imse = cv_result[ml_key][-1]
    ...
    for i in range(cv_times):
        ...
        for j in range(1, exfeature_num + 1):
            ...
            if params_dict['objective'] == 'regression':
                l2_key = [k for k in cv_result.keys() if 'l2-mean' in k][0]
                jmse = cv_result[l2_key][-1]
            else:
                ml_key = [k for k in cv_result.keys() if 'multi_logloss-mean' in k][0]
                jmse = cv_result[ml_key][-1]
